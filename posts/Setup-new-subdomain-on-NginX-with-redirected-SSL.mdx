...
First enter the subdomain, then enter port if you want this subdomain to proxy to a localhost server, or an absolute path if it is a static file hosting.

<WidgetsRoot defaultValues={{subdomain: "subdomain.dipsy.me", port: "3000"}}>
<div className="flex space-x-3">
	<WidgetInput name="subdomain" />
	<WidgetInput name="port" />
</div>
...
2. Run <code>sudo vi /etc/nginx/conf.d/<WidgetValue name="subdomain" /></code> and copy past below
