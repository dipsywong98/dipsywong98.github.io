...
<WidgetsRoot defaultValues={{subdomain: "subdomain.dipsy.me"}} names={['subdomain']}>
<WidgetInput name="subdomain" defaultValue="subdomain.dipsy.me" />
<WidgetValue name="subdomain" />

export const amount = 123

hello 

<CodeBlock language="sh">echo %[subdomain]% </CodeBlock>

1. run certbot
	```sh
	sudo certbot --nginx -d mydomain.com -d mysubdomain.mydomain.com
	```
1. go `/etc/nginx/sites-available/default` <WidgetValue name="subdomain" />
1. edit and paste
	```nginx
	server {
